...
class ClientThread(Thread):
    ...
    def run(self):
        status = 0  # 0 for offline , 1 for online , 2 for blocked
        data2 = "successful"
        self.socket.send(data2)

        self.socket.send(bytes(int(TIME_OUT)))
...
class ClientThreadread(Thread):
    ...
    def run(self):
        ...
        welcomemsg = "Hello sir!"
        conn2.send(str.encode(welcomemsg))
        chat = "initial"
        print("ind here is")
...
sendqueues = {}
TCP_IP = '127.0.0.1'  # IP to connect to
TCP_PORT = int(sys.argv[1])  # Port to connect to - taken from arguments
TCP_PORT2 = 125
